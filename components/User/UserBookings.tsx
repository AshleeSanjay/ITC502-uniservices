...
const UserBookings: React.FC = () => {
  ...
  const [dateFilter, setDateFilter] = useState<"upcoming" | "passed" | null>(
    null
  );
      ...
      <div className="flex space-x-2 mb-4">
        <button
          className={`rounded-full px-4 py-2 text-sm font-medium border ${
            filter === null ? "bg-blue-500 text-white" : "text-blue-500"
          }`}
        ...
        <button
          className={`rounded-full px-4 py-2 text-sm font-medium border ${
            dateFilter === "passed" ? "bg-red-500 text-white" : "text-red-500"
          }`}
          onClick={() =>
            setDateFilter(dateFilter === "passed" ? null : "passed")
          }
        >
          Passed
        </button>
        ...
        {bookingTypes.map((type) => (
          <button
            key={type}
            className={`rounded-full px-4 py-2 text-sm font-medium border ${
              filter === type ? "bg-blue-500 text-white" : "text-blue-500"
            }`}
      ...
      <table className="min-w-full divide-y divide-gray-200 rounded-xl">
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {bookings
            .filter(
              (booking) =>
                (filter === null || booking.bookingType === filter) &&
                (dateFilter === null ||
            ...
            .map((booking) => (
              ...
              >
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                  {booking.dropdownOption}
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-sm flex justify-center text-gray-500">
                  <span
                    className="flex items-center justify-center h-6 w-6 rounded-full bg-green-500"
                    style={{
                      backgroundColor: isUpcoming(booking.date)
                        ? "#10B981"
                        : "#EF4444",
                    }}
                  >
